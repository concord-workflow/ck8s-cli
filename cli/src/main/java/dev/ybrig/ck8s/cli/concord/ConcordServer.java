...
public class ConcordServer {
    ...
    public static HttpServer start()
        ...
        var server = HttpServer.create(new InetSocketAddress("0.0.0.0", 8001), 0);
        Runtime.getRuntime().addShutdownHook(new Thread(() -> server.stop(0)));

        var handler = RuntimeDelegate.getInstance().createEndpoint(new ConcordResourceApp(), HttpHandler.class);
    ...
    private static void assertPort(int port) {
        try (var unused = new ServerSocket(port, 50, InetAddress.getByName("0.0.0.0"))) {
            // do nothing
